...
def action(gesture):
    ...
    if gesture == 'S':
        requests.get(url = URL + "/brightUp", params = {})
        print("S is the one")

    elif gesture == 'R' or gesture == 'X':
        requests.get(url = URL + "/brightDown", params = {})
        print("R/X is the one")

    elif gesture == 'Q':
        requests.get(url = URL + "/volUp", params = {})
        print("Q is the one")

    elif gesture == 'O':
        requests.get(url = URL + "/volDown", params = {})
        print("O is the one")
...
class GestureRecognizer(object):
    ...
    def recognize_gesture(self, image):
        ...
        # croppedImage = resize(image, (128,128))
        hogvec = convertToGrayToHOG(croppedImage)
        io.imsave('blah.jpg',croppedImage)
...
def main():
    ...
    # gs = GestureRecognizer('/home/aravind/gesture/dataset/', 'handDetector.pkl', 'signDetector.pkl')
    ...
    # gs.save_model(name = "Agents-of-Chaos.pkl.gz", version = "0.0.1", author = 'Agents')
    new_gr = GestureRecognizer.load_model(name = "Agents-of-Chaos.pkl.gz") # automatic dict unpacking
    
    while True:
        os.system("fswebcam -d /dev/video1 -D 1 --jpeg 85 sample.jpg -b")
        img = io.imread('sample.jpg')
        img_resized = resize(img, (240, 320))
        # print gs.recognize_gesture(img_resized)
        recognized = new_gr.recognize_gesture(img_resized)
        print recognized
...
def main():
    ...
    gs = GestureRecognizer('/home/aravind/gesture/dataset/')
    # userlist=['user_shanky']
    userlist=[ 'user_10', 'user_3', 'user_4', 'user_5',  'user_6', 'user_7', 'user_9']
    user_tr = userlist[:1]
    user_te = userlist[-1:]
    gs.train(user_tr)
